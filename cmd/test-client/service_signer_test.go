...
func TestServiceToSignerCommunication(ctx context.Context, signerEndpoint string) error {
	log.Info().Str("endpoint", signerEndpoint).Msg("Testing Service -> Signer communication (simulating Service gRPC client)...")

	// 创建 gRPC 连接（模拟 Service 客户端，测试环境使用 insecure）
	conn, err := grpc.Dial(signerEndpoint, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
