...
type InfrastructureServer struct {
	...
	deliveryStateMachine *backup.StateMachine
	store                backup.Store
...
func NewInfrastructureServer(
	...
	jwtManager := auth.NewJWTManager(cfg.MPC.JWTSecret, "go-mpc-wallet", cfg.MPC.JWTDuration)
	...
	return &InfrastructureServer{
		...
		deliveryStateMachine: backup.NewStateMachine(store),
		cfg:                  cfg,
...
func (s *InfrastructureServer) Start(ctx context.Context) error {
	// 如果启用了 TLS，在启动前验证证书
	if s.cfg.MPC.TLSEnabled {
		if err := cert.VerifyTLSConfig(s.cfg.MPC.TLSCertFile, s.cfg.MPC.TLSKeyFile, s.cfg.MPC.TLSCACertFile); err != nil {
