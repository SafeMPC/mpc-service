...
func testNodesExists(t *testing.T) {
	...
	e, err := NodeExists(ctx, tx, o.NodeID)
	if err != nil {
...
func testNodesFind(t *testing.T) {
	...
	nodeFound, err := FindNode(ctx, tx, o.NodeID)
	if err != nil {
		t.Error(err)
	}
...
func testNodesInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(nodePrimaryKeyColumns, nodeColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
