...
build: ##- Default 'make' target: sql, swagger, go-generate-handlers, go-format, go-build and lint.
	...
	@$(MAKE) go-format
	@$(MAKE) go-build
	# @$(MAKE) lint  # Temporarily disabled
...
swagger-lint:
	...
	@spectral lint --fail-severity=error api/swagger.yml
...
swagger-concat: ##- (opt) Regenerates api/swagger.yml based on api/paths/*.
	...
	@# Fix address examples that lost quotes during flatten (swagger lint requires string type)
	@# Match unquoted hex addresses (any length starting with 0x)
	@# Only match if not already quoted to avoid breaking existing quotes
	@if command -v gsed >/dev/null 2>&1; then \
		gsed -i '/^[[:space:]]*example: 0x[0-9a-fA-F]\+$$/s/example: 0x\(.*\)$$/example: "0x\1"/g' api/swagger.yml; \
	else \
		sed -i.bak '/^[[:space:]]*example: 0x[0-9a-fA-F]\+$$/s/example: 0x\(.*\)$$/example: "0x\1"/g' api/swagger.yml 2>/dev/null || sed -i '/^[[:space:]]*example: 0x[0-9a-fA-F]\+$$/s/example: 0x\(.*\)$$/example: "0x\1"/g' api/swagger.yml; \
		rm -f api/swagger.yml.bak 2>/dev/null || true; \
	fi
